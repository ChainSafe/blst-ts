...
class BlstTsAddon : public Napi::Addon<BlstTsAddon> {
   ...
   public:
     ...
     *
     * @param[out] bytes    Output array for generated bytes
     * @param[in]  length   Length of bytes to be generated. Must be less than
     * or equal to the length of the output bytes array
     *
     * @return {bool}   True for valid generation, false for error. Must not
     *                  discard return value.
     */
    [[nodiscard]] bool GetRandomNonZeroBytes(blst::byte *bytes, size_t length);
};
