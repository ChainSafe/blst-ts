...
Napi::Value SecretKey::FromKeygen(const Napi::CallbackInfo &info) {
    ...
    BLST_TS_UNWRAP_UINT_8_ARRAY(ikm_value, ikm, "ikm", scope.Escape(env.Undefined()))
...
Napi::Value SecretKey::Deserialize(const Napi::CallbackInfo &info) {
    ...
    BLST_TS_UNWRAP_UINT_8_ARRAY(sk_bytes_value, sk_bytes, "skBytes", scope.Escape(env.Undefined()))
