...
export class BlsMultiThreadNaive {
  ...
  async verify(msg: Uint8Array, pk: bls.PublicKey, sig: bls.Signature): Promise<boolean> {
    return this.pool.queue((worker) => worker.verify(msg, pk.serialize(), sig.serialize()));
  }

  async verifyMultipleAggregateSignatures(sets: bls.SignatureSet[]): Promise<boolean> {
  ...
  async receive(msgs: Uint8Array[], pks: Uint8Array[], sigs: Uint8Array[]): Promise<void> {
    await this.pool.queue((worker) => worker.receive(msgs, pks, sigs));
  }

  async serders(msgs: Uint8Array[], pks: bls.PublicKey[], sigs: bls.Signature[]): Promise<void> {
